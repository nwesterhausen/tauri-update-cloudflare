import { ARCH_FILTERS, AVAILABLE_ARCHITECTURES, AVAILABLE_PLATFORMS, PLATFORM_FILTERS } from './constants';
import { fileExt } from './utils/fileExt';

export const testAsset = (target: AVAILABLE_PLATFORMS, arch: AVAILABLE_ARCHITECTURES, fileName: string): boolean => {
  const { matches, extension } = PLATFORM_FILTERS[target];
  const arch_matches = ARCH_FILTERS[arch];
  const rightArch = arch_matches && ARCH_FILTERS[arch].some((arch) => fileName.includes(arch));
