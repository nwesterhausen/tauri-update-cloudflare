...
export const getReleases = async (request: Request): Promise<Response> => {
  const reqUrl = new URL(`https://api.github.com/repos/${GITHUB_ACCOUNT}/${GITHUB_REPO}/releases/latest`);
...
export async function findAssetSignature(fileName: string, assets: Asset[]): Promise<string | undefined> {
